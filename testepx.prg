...
PROCEDURE testecriacaoeappend()
   ...
   Local aData := { ;
                    { "00001", "1", "Tabela Alfa de Teste", 12.50, 30, "2026-12-31", .T. }, ;
                    { "00002", "2", "Tabela Beta Especial" ,  5.00, 15, "2026-06-15", .F. }, ;
                    { "00003", "1", "Tabela Gama Padrao"  , 100.00, 60, "2027-01-01", .T. }  ;
                  }

   Local pPxDoc := NIL
   Local i

   ? "Iniciando criacao e gravacao do arquivo Paradox (.db)..."
   ...
   // ETAPA 1: Criação da Estrutura e do Arquivo Vazio
   IF PX_Create_Table( pPxDoc, cDbFile, aStruct ) == 0
      ? "Estrutura do Paradox criada com sucesso em: " + cDbFile
      
      // Fecha e reabre o documento para sincronizar os ponteiros internos da pxlib
      PX_Close( pPxDoc )
      
      IF PX_Open_File( pPxDoc, cDbFile ) == 0
         ? "Arquivo reaberto para gravacao com sucesso."

         // ETAPA 2: Inclusão de Dados (Linha por Linha)
         FOR i := 1 TO Len( aData )
            IF PX_Append_Record( pPxDoc, aData[i] ) == 0
               ? "Registro " + AllTrim( Str( i ) ) + " inserido com sucesso."
            ELSE
               ? "Erro ao inserir o registro " + AllTrim( Str( i ) )
            ENDIF
         NEXT
      ELSE
         ? "Erro ao reabrir o arquivo Paradox para insercao."
      ENDIF
   ELSE
      ? "Erro ao criar o arquivo Paradox."
   ENDIF
...
PROCEDURE testedbparadbf()
   Local cDbFile  := "clientes.db"
   Local cDbfFile := "clientes_convertido.dbf"
   Local pPxDoc   := NIL
  ...
  // use ba01
  // hb_memowrit("uso.txt",hb_valtostr(dbstruct()))
  // quit
   ...
   IF PX_Open_File( pPxDoc, cDbFile ) == 0
      ...
      DbCreate( cDbfFile, aStruct, "DBFCDX", .T., "TRG" )
...
// Função auxiliar em C para retornar o tipo, tamanho e decimais do campo do Paradox
HB_FUNC( PX_GET_FIELD_TYPE_AND_LEN )
{
   pxdoc_t *pxdoc = (pxdoc_t *) (HB_PTRDIFF) hb_parnl( 1 );
   int fieldno = hb_parni( 2 );
   ...
   if( pxdoc && pxdoc->px_head && fieldno >= 0 && fieldno < pxdoc->px_head->px_numfields ) {
      ...
      // Retorna o tamanho (len) por referência no parâmetro 3
      hb_storni( (int) field->px_flen, 3 );
      // Retorna as casas decimais (px_fdc) por referência no parâmetro 4
      hb_storni( (int) field->px_fdc, 4 );
      
      // Retorna o tipo numérico do campo (ftype)
      hb_retni( field->px_ftype );
   } else {
      hb_retni( 0 );
   }
}
